...
def current_stats():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT socPercent, wattsOut, wattsIn, temperature, whStored, timeToEmptyFull
        FROM logs 
...
def charge_profile():
    ...
    try:
        r = requests.get(f"http://{yeti_ip}/state", timeout=3)
        data = r.json()
        profile = data.get("chargeProfile", {})
        return jsonify({
            "min": profile.get("min", None),
            "max": profile.get("max", None),
            "re":  profile.get("re",  None)
        })
    except Exception as e:
        return jsonify({"error": "Yeti unreachable"}), 503

@app.route('/api/set_charge_profile', methods=['POST'])
@login_required
@admin_required
...
.battery-modes-only .mode-item:hover {
    background: rgba(0, 217, 255, 0.1);
    border-color: rgba(0, 217, 255, 0.3);
}

.battery-modes-only .mode-item.active {

    border-color: #00d9ff;

}

.battery-modes-only .mode-item-header {
    display: flex;
    align-items: center;
    gap: 15px;
}

.battery-modes-only .mode-indicator-dot {
    width: 18px;
    height: 18px;
    border-radius: 50%;
...
.battery-modes-only .mode-item.active .mode-indicator-dot {
    background: #00d9ff;
    border-color: #00d9ff;
    box-shadow: 0 0 12px rgba(0, 217, 255, 0.8);
}

.battery-modes-only .mode-title {
    color: #e0e6ed;
    font-size: 14px;
    font-weight: 600;
    margin-bottom: 4px;
...
.battery-modes-only .mode-range {
    color: #00d9ff;
    font-size: 13px;
    font-weight: bold;
}

.battery-modes-only .mode-details {
    margin-top: 15px;
    padding-top: 15px;
...
.battery-modes-only .mode-details .detail-note {
    grid-column: 1 / -1;
    color: #00d9ff;
    margin-top: 6px;
    font-style: italic;
...
<!-- Section Modes Batterie -->
<h2 style="color: #00d9ff; width: 100%; text-align: center; margin: 20px; display: flex; align-items: center; justify-content: center; gap: 12px;">
    MODES DE CHARGE DE LA BATTERIE
    <span id="batteryArrow" onclick="toggleBatteryDetails()" style="cursor:pointer; font-size:20px; display:inline-block; transition: transform 0.3s;">▶</span>
</h2>

{% if user_role != 'admin' %}
<div class="access-denied" style="max-width:1400px; margin: 0 auto 15px;">
    ⚠️ Vous êtes en mode lecture seule. Seuls les administrateurs peuvent modifier le mode de charge.
</div>
{% endif %}

<div class="battery-modes-only">

    <!-- MODE ECO -->
    <div class="mode-item" data-mode="eco"
         onclick="{% if user_role == 'admin' %}applyBatteryMode('eco'){% else %}showAccessDenied(){% endif %}">
        <div class="mode-item-header">
            <div class="mode-indicator-dot"></div>
            <div class="mode-content">
                <div class="mode-title">Mode économie d'énergie</div>
                <div class="mode-range">20% - 85%</div>
            </div>
        </div>
        <div class="mode-details" id="details-eco">
            <div>🔋 <strong>Minimum décharge</strong></div>
            <div>20% — Sorties désactivées à 20%</div>
            <div>🔁 <strong>Point de recharge</strong></div>
            <div>25% — Recharge recommence à 25%</div>
            <div>✅ <strong>Charge maximale</strong></div>
            <div>85% — Arrête la charge à 85%</div>
            <div class="detail-note">Préserve la durée de vie. Idéal pour usage quotidien.</div>
        </div>
    </div>

    <!-- MODE STANDARD -->
    <div class="mode-item" data-mode="standard"
         onclick="{% if user_role == 'admin' %}applyBatteryMode('standard'){% else %}showAccessDenied(){% endif %}">
        <div class="mode-item-header">
            <div class="mode-indicator-dot"></div>
            <div class="mode-content">
                <div class="mode-title">Mode standard</div>
                <div class="mode-range">0% - 100%</div>
            </div>
        </div>
        <div class="mode-details" id="details-standard">
            <div>🔋 <strong>Minimum décharge</strong></div>
            <div>0% — Décharge complète</div>
            <div>🔁 <strong>Point de recharge</strong></div>
            <div>95% — Recharge recommence à 95%</div>
            <div>✅ <strong>Charge maximale</strong></div>
            <div>100% — Charge complète</div>
            <div class="detail-note">Capacité totale. Idéal pour événements hors réseau.</div>
        </div>
    </div>

    <!-- MODE PERSONNALISÉ -->
    <div class="mode-item" data-mode="custom"
         onclick="{% if user_role == 'admin' %}applyBatteryMode('custom'){% else %}showAccessDenied(){% endif %}">
        <div class="mode-item-header">
            <div class="mode-indicator-dot"></div>
            <div class="mode-content">
                <div class="mode-title">Mode personnalisé</div>
                <div class="mode-range" id="custom-range-label">-- % - -- %</div>
            </div>
        </div>
        <div class="mode-details" id="details-custom">
            <div>🔋 <strong>Minimum décharge</strong></div>
            <div id="custom-min-display">-- %</div>
            <div>🔁 <strong>Point de recharge</strong></div>
            <div id="custom-re-display">-- %</div>
            <div>✅ <strong>Charge maximale</strong></div>
            <div id="custom-max-display">-- %</div>
            <div class="detail-note">Profil adapté à vos besoins spécifiques.</div>
            {% if user_role == 'admin' %}
            <div style="grid-column: 1 / -1; margin-top: 12px;">
                <button onclick="event.stopPropagation(); openCustomModal()"
                        style="width:100%; padding:8px; background:rgba(0,217,255,0.2);
                               border:1px solid #00d9ff; border-radius:8px; color:#00d9ff;
...
<!-- Modal saisie valeurs personnalisées -->
{% if user_role == 'admin' %}
<div id="customModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%;
     background:rgba(0,0,0,0.7); z-index:9999; justify-content:center; align-items:center;">
    <div style="background:#1a2332; border:1px solid #00d9ff; border-radius:16px;
                padding:30px; min-width:300px; box-shadow:0 8px 32px rgba(0,0,0,0.6);">
        <h3 style="color:#00d9ff; text-align:center; margin-bottom:20px;">⚙️ Valeurs personnalisées</h3>
        <div style="display:flex; flex-direction:column; gap:15px;">
            <label style="color:#a0aec0; font-size:13px;">
                🔋 Minimum décharge (%)
                <input id="input-min" type="number" min="0" max="100"
                       style="width:100%; margin-top:5px; padding:8px; background:rgba(0,0,0,0.3);
                              border:1px solid rgba(255,255,255,0.2); border-radius:6px; color:#fff; font-size:14px;">
            </label>
            <label style="color:#a0aec0; font-size:13px;">
                🔁 Point de recharge (%)
                <input id="input-re" type="number" min="0" max="100"
                       style="width:100%; margin-top:5px; padding:8px; background:rgba(0,0,0,0.3);
                              border:1px solid rgba(255,255,255,0.2); border-radius:6px; color:#fff; font-size:14px;">
            </label>
            <label style="color:#a0aec0; font-size:13px;">
                ✅ Charge maximale (%)
                <input id="input-max" type="number" min="0" max="100"
                       style="width:100%; margin-top:5px; padding:8px; background:rgba(0,0,0,0.3);
                              border:1px solid rgba(255,255,255,0.2); border-radius:6px; color:#fff; font-size:14px;">
            </label>
        </div>
        <div style="display:flex; gap:10px; margin-top:20px;">
            <button onclick="closeCustomModal()"
                    style="flex:1; padding:10px; background:rgba(255,85,85,0.2); border:1px solid #ff5555;
                           border-radius:8px; color:#ff5555; cursor:pointer; font-weight:bold;">
                Annuler
            </button>
            <button onclick="saveCustomProfile()"
                    style="flex:1; padding:10px; background:linear-gradient(135deg,#00d9ff,#0099cc);
...
{% endif %}
    ...
    <div class="output-ports">
        ...
        <div class="ports-timer-row">
            ...
            <div class="timer-box-ports">
                <div class="timer-label-ports">Temps de charge<br>ou de décharge</div>
                <div class="timer-display-ports" id="timeToFullEmpty">--:--</div>
            </div>
        </div>
    </div>
...
function updateStats() {
    fetch('/api/current_stats')
        .then(r => r.json())
        .then(data => {
            const elOut  = document.getElementById('stat-watts-out');
            const elIn   = document.getElementById('stat-watts-in');
            const elTemp = document.getElementById('stat-temp');
            if (elOut)  elOut.textContent  = Math.round(data.watts_out) + 'W';
            if (elIn)   elIn.textContent   = Math.round(data.watts_in) + 'W';
            if (elTemp) elTemp.textContent = data.temp.toFixed(1) + '°C';
            ...
            // ⏱️ Timer
            const timerEl = document.getElementById('timeToFullEmpty');
            const labelEl = document.querySelector('.timer-label-ports');
            if (timerEl) {
                if (!data.connected) {
                    timerEl.textContent = '--:--';
                } else {
                    const rawTime = data.time_to_empty_full;
                    if (rawTime !== null && rawTime !== undefined && rawTime !== -1) {
                        const minutes = Math.floor(Math.abs(rawTime));
            ...
            if (labelEl) {
                if (!data.connected) {
                    labelEl.innerHTML = 'Temps de charge<br>ou de décharge';
                } else if (data.watts_in > data.watts_out) {
                    labelEl.innerHTML = '⚡ Temps avant<br>pleine charge';
                } else if (data.watts_out > 0) {
                    labelEl.innerHTML = '🔋 Temps avant<br>décharge complète';
                } else {
                    labelEl.innerHTML = 'Temps de charge<br>ou de décharge';
                }
            }
...
let currentBatteryMode = 'eco';
let currentProfile = { min: null, max: null, re: null };

function toggleBatteryDetails() {
    detailsVisible = !detailsVisible;
    document.querySelectorAll('.mode-details').forEach(d => {
        d.style.display = detailsVisible ? 'grid' : 'none';
    });
    document.getElementById('batteryArrow').style.transform = detailsVisible ? 'rotate(90deg)' : 'rotate(0deg)';
}

function selectBatteryMode(mode) {
...
function applyBatteryMode(mode) {
    if (mode === 'custom') {
        selectBatteryMode('custom');
        return;
    }
    let profile;
    if (mode === 'eco')      profile = { min: 20, max: 85, re: 25 };
    if (mode === 'standard') profile = { min: 0,  max: 100, re: 95 };
    fetch('/api/set_charge_profile', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
    ...
    .then(res => {
        if (res.success) {
            selectBatteryMode(mode);
            currentProfile = profile;
            updateCustomDisplay(profile.min, profile.max, profile.re);
        } else {
            alert('Erreur lors du changement de mode');
        }
    })
    .catch(() => alert('Erreur de connexion au Yeti'));
}

function openCustomModal() {
    document.getElementById('input-min').value = currentProfile.min ?? '';
    document.getElementById('input-re').value  = currentProfile.re  ?? '';
    document.getElementById('input-max').value = currentProfile.max ?? '';
...
function saveCustomProfile() {
    ...
    const re  = parseInt(document.getElementById('input-re').value);
    const max = parseInt(document.getElementById('input-max').value);
    if (isNaN(min) || isNaN(re) || isNaN(max)) {
    ...
    fetch('/api/set_charge_profile', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ min, max, re })
    })
    .then(r => r.json())
    .then(res => {
        if (res.success) {
            currentProfile = { min, max, re };
            updateCustomDisplay(min, max, re);
            selectBatteryMode('custom');
            closeCustomModal();
        } else {
...
function updateCustomDisplay(min, max, re) {
    const elMin   = document.getElementById('custom-min-display');
    const elRe    = document.getElementById('custom-re-display');
    const elMax   = document.getElementById('custom-max-display');
    const elRange = document.getElementById('custom-range-label');
    if (elMin)   elMin.textContent   = min + ' %';
    if (elRe)    elRe.textContent    = re  + ' %';
    if (elMax)   elMax.textContent   = max + ' %';
    if (elRange) elRange.textContent = min + '% - ' + max + '%';
}
...
function refreshBatteryMode() {
    fetch('/api/current_stats')
        ...
        .then(stats => {
            if (!stats.connected) {
                document.querySelectorAll('.mode-item').forEach(el => el.classList.remove('active'));
                currentBatteryMode = null;
                return;
            }
            return fetch('/api/charge_profile')
                .then(r => {
                    if (!r.ok) return null;
                    return r.json();
                })
                .then(profile => {
                    ...
                    const re  = profile.re;
                    if (min === null || max === null || re === null) return;
                    currentProfile = { min, max, re };
                    updateCustomDisplay(min, max, re);
                    let detected = 'custom';
                    if (min === 20 && max === 85 && re === 25)  detected = 'eco';
                    if (min === 0  && max === 100 && re === 95) detected = 'standard';
                    selectBatteryMode(detected);
                });
        })
        .catch(() => {
            document.querySelectorAll('.mode-item').forEach(el => el.classList.remove('active'));
...
refreshBatteryMode();
setInterval(refreshBatteryMode, 30000);
